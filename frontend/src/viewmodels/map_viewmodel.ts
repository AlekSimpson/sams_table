...
export function map_viewmodel() {
  ...
  const activate_map = useCallback(
    async (_map_ID: string) => {
      // TODO: send map_activated WS event with full tile list; server broadcasts to room
  ...
  const load_map = useCallback(
    async (map_ID: string) => {
      // TODO: const tiles = await mapApi.getTiles(mapID); syncTiles(tiles)
      const _tiles = await map_api.getTiles(map_ID)
      void sync_tiles
      throw new Error('not implemented')
    },
    [sync_tiles]
